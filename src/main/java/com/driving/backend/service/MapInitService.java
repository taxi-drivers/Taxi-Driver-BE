...
public class MapInitService {
    ...
    public MapInitResponse init(
        ...
        LevelRule activeRule = levelRuleRepository.findFirstByIsActiveTrueOrderByLevelRuleIdDesc()
                .orElse(null);
        ...
        boolean shouldIncludeSegments = parseIncludeSegments(includeSegments);
        List<MapSegmentItem> segments;
        if (shouldIncludeSegments) {
            MapSegmentsResponse segmentsResponse = mapSegmentService.getSegments(
                    minLat,
                    minLon,
                    maxLat,
                    maxLon,
